...
- 라이브 주소(Vercel): https://group-maker-five.vercel.app/
- 배포 브랜치: `master` 브랜치를 Vercel이 GitHub 연동으로 감시하며, push할 때마다 자동 배포

> 이전에는 GitHub Pages(`https://tarrence-lee.github.io/group-maker/`)로 서빙했으나, 2026-07-13에 Vercel로
> 이전하며 GitHub Pages는 비활성화했습니다.

## 배포 구조
Vercel은 커스텀 도메인(`*.vercel.app`) 루트에서 서빙합니다. 이전 GitHub Pages 프로젝트 사이트(하위 경로)에서도
동작하도록 `manifest.json`/`service-worker.js`/아이콘 경로가 전부 **상대 경로**로 작성되어 있어
(`start_url: "./index.html"`, `scope: "./"` 등) 별도 수정 없이 그대로 동작합니다.

빌드 명령/출력 디렉토리 없이 정적 파일을 그대로 서빙합니다(Framework Preset: Other). `vercel.json`에서
`service-worker.js`/`manifest.json`은 항상 재검증하도록(`no-cache`), 아이콘은 1년간 캐시하도록
(`immutable`) 헤더를 지정해 두었습니다.
...
3. 변경 사항을 커밋하고 `master` 브랜치에 푸시하면 Vercel이 자동으로 다시 빌드/배포합니다.
   ```
   git add -A
   git commit -m "설명"
   git push
   ```
4. 배포는 보통 1분 이내에 반영됩니다. 진행 상황은 https://vercel.com/dashboard 의 프로젝트 페이지에서
   확인할 수 있습니다.
